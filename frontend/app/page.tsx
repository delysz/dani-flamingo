...
const Globe = dynamic(() => import('react-globe.gl'), { 
  ssr: false,
  loading: () => <div className="w-full h-[600px] flex items-center justify-center opacity-30 tracking-[0.3em] text-xs">INITIALIZING ORBIT...</div>
});
...
// --- GLOBO 3D (Corregido) ---
const EpicGlobe3D = ({ color }: { color: string }) => {
  ...
  const arcsData = useMemo(() => Array.from({ length: 12 }, () => ({
  ...
  return (
    <div className="relative w-full h-[500px] md:h-[700px] rounded-3xl overflow-hidden border border-white/10 bg-black/40 backdrop-blur-sm">
      <div className="absolute top-6 left-6 z-10">
        <h3 className="text-xl font-bold uppercase tracking-widest flex items-center gap-2" style={{ color }}>
          <Globe2 className="w-5 h-5" /> Global Coverage
        </h3>
        <p className="text-xs text-white/50 mt-1">Interactive Visualization</p>
      </div>
        ...
        <button 
          onClick={() => {
            if (globeRef.current) {
              ...
              globeRef.current.controls().autoRotate = nextState;
            }
          }}
          className="p-3 rounded-full bg-black/50 border border-white/10 text-white/70 hover:text-white hover:bg-black/80 transition-all"
        >
          {isRotating ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
        </button>
        <button 
          onClick={() => globeRef.current?.pointOfView({ lat: 20, lng: 0, altitude: 2.2 }, 1000)}
          className="p-3 rounded-full bg-black/50 border border-white/10 text-white/70 hover:text-white hover:bg-black/80 transition-all"
        >
      ...
      <Globe
        ...
        // Anillos (Efecto Pulse Reemplazado)
        ringsData={cities}
...
const PhotoCard = ({ photo, index, themeColor, delay }: any) => {
  return (
    ...
    >
      <div className="relative aspect-[4/5] bg-[#0a0a0a] overflow-hidden border border-white/10 transition-all duration-700 hover:border-white/30">
        <Image 
          src={photo.imageUrl} 
          alt={photo.title} 
          fill
        ...
        <div className="absolute inset-x-0 bottom-0 p-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-start">
    ...
    <main className="min-h-screen bg-[#050505] text-white overflow-x-hidden selection:bg-white/20">
      ...
      <div className="relative z-10 w-full min-h-screen px-6 md:px-16 lg:px-24 flex flex-col items-center">
        ...
        <header className="w-full max-w-7xl mx-auto flex flex-col items-center justify-center min-h-[70vh] relative mb-12">
          ...
          >
             <div>
                <div className="text-3xl font-bold text-white"><Counter to={50} />+</div>
                <div className="text-[10px] uppercase tracking-widest text-white/40 mt-1">Countries</div>
             </div>
             <div className="w-[1px] h-10 bg-white/10" />
             <div>
                <div className="text-3xl font-bold text-white"><Counter to={filtered.length} /></div>
